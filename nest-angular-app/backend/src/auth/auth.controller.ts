// Nest
import { Controller, Get, Res, Query, Req, UseGuards, Post, Body, HttpException, HttpStatus } from '@nestjs/common';
...
export class AuthController {
	...
	authRedirect(@Res() res): any {
		return res.redirect(`https://api.intra.42.fr/oauth/authorize?client_id=9ff20d31a6e021e5a2010628c4c7bc1604fd35f5284b13662bdf431c3d77bbb8&redirect_uri=${urlRedirect}&response_type=code`);
	}
	...
	is_administrator(idIntra: string) {
		return (idIntra === 'mframbou' || idIntra === 'dsamain' || idIntra === 'oronda' || idIntra === 'sspina');
	}
		...
		try {
			return fetch('https://api.intra.42.fr/v2/me', {
				...
				.then(async ret => {
					...
					let session: any = await this.sessionService.find_session(user.idIntra);
			...
			throw new HttpException("It didn't works " + e.message, HttpStatus.CONFLICT)
		}
	}
	...
	async check_two_factor(@Body() body, @Res() res:Response): Promise<any> {
		this.authService.check_two_factor(body, res)
			.then((e) => {
